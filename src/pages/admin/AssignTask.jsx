...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "16WjX6Zb-y1BLLLPpImhCHj4tyAgEKlyZ07WmM-HgCE8";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/16WjX6Zb-y1BLLLPpImhCHj4tyAgEKlyZ07WmM-HgCE8/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "16WjX6Zb-y1BLLLPpImhCHj4tyAgEKlyZ07WmM-HgCE8";
